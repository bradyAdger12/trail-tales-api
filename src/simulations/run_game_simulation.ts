...
import { Activity, Game, GameDifficulty } from "@prisma/client";
...
import { advanceSurvivalDay, getDuration } from "../routes/survival_day/survival_day.controller";
...
async function runSimulation(fastify: FastifyInstance, difficulty: string) {
    try {
        ...
        let game = null
        await startGame(user.id, difficulty as GameDifficulty)
        for (let i = 0; i < DAYS_TO_SURVIVE; i++) {
            ...
            if (randomActivityDuration !== 'rest') {
                const activityJson = {
                    ...
                    elapsed_time_in_seconds: getDuration(game!, randomActivityDuration as GameDifficulty),
                    source: 'strava',
